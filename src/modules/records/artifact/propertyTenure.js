 ...
 */
  setSpatialUnits (spatialUnits) {
    if (spatialUnits) {
      ...
      if (spatialUnits.length > 0) { this.artifact.details.spatialUnits = spatialUnits }
    }
  }
    ...
    if (parties) {
      ...
      if (parties.length > 0) {
        ...
        for (let party of parties) {
          if (!party.role) { throw new Error(`Objects in parties must contain role property.`) }
          if (!party.party) { throw new Error(`Objects in parties must contain party property.`) }
  ...
  setAttributes (attrs) {
    if (attrs) {
      ...
      for (let attr in attrs) {
        if (attrs[attr]) { this.artifact.details.attrs[attr] = attrs[attr] }
      }
    }
  }
}
