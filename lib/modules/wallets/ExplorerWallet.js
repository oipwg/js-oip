...
class ExplorerWallet {
  ...
  constructor(options) {
    ...
    if (options.explorerUrl) {
      network.explorer = new _insightExplorer.Insight(options.explorerUrl);
    }
    ...
    this.ECPair = _bitcoinjsLib.default.ECPair.fromWIF(options.wif, this.network);
  ...
  createManualUtxos() {
    ...
    for (let f of floTxs) {
      ...
      for (let i = 0; i < outputs.length; i++) {
        ...
        let _bitcoin$address$from = _bitcoinjsLib.default.address.fromBase58Check(addr),
            hash = _bitcoin$address$from.hash;

        let testnetAddr = _bitcoinjsLib.default.address.toBase58Check(hash, 115);
