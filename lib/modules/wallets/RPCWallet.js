...
const TX_FEE_PER_BYTE = 0.0000001; // Average size of tx data (without floData) to calculate min txFee

const TX_AVG_BYTE_SIZE = 200;
const SAT_PER_FLO = 100000000; // The minimum amount a utxo is allowed to be for us to use

const MIN_UTXO_AMOUNT = .001; // Prevent chaining over ancestor limit
...
class RPCWallet {
      ...
      if (getMempoolEntry.error && getMempoolEntry.error !== null) {
        ...
        if (getMempoolEntry.error.message === 'Transaction not in mempool' || getMempoolEntry.error.message === 'Transaction not in mempool.') {
          ...
          if (confirmations >= 1) {
            ...
            console.error("TXID not in mempool AND not found! ".concat(txidToCheck));
        ...
        foundUnconfirmed = true;
        console.log("Oldest TX in mempool ".concat(txidToCheck));
      }
    }

    console.log("[RPC Wallet] ".concat(numberConfirmed, " Transactions Confirmed!"));
  ...
  async rebroadcastTransactions() {
    ...
    console.log("[RPC Wallet] Announcing ".concat(this.unconfirmedTransactions.length, " transactions to Peers...")); // Destroy any peers we have currently connected to to get a "clean" rebroadcast
  ...
  async sendDataToChain(floData, previousTransactionOutput) {
    ...
    let output = {};
    output[this.publicAddress] = parseFloat((inputAmount - myTxFee).toFixed(8)); // Send the transaction

    let transactionOutput = await this.sendTX([input], output, floData); // Returns the TXID of the transaction if sent successfully
  ...
  async prepSignedTXforChain(floData, previousTransactionOutput) {
    ...
    let inputAmount = previousTransactionOutput !== undefined ? previousTransactionOutput.amount : utxos[0].amount; // Calculate the minimum Transaction fee for our transaction by counting the size of the inputs, outputs, and floData
    let myTxFee = (this.options.txFeePerByte || TX_FEE_PER_BYTE) * (TX_AVG_BYTE_SIZE + (0, _util.varIntBuffer)(floData.length).toString('hex').length + Buffer.from(floData).length); // Create an output to send the funds to
    let output = {};
   ...
   */
   ...
   async makeTX(inputs, outputs, floData) {

    // console.log('[RPC Wallet] Making transaction with this data: ', floData, inputs, outputs);
    // Perform validation checks on the floData being added to the chain
    if (typeof floData !== 'string') {
      throw new Error("Data must be of type string. Got: ".concat(typeof floData));
    }
  ...
  async checkForConfirmations() {
    ...
    console.log("[RPC Wallet] Checking ".concat(this.getConfirmationSubscriptionCount(), " confirmation subscriptions..."));
    ...
    for (let subscription in this.onConfirmationSubscriptions) {
      let {
        ...
        record,
        options
      } = this.onConfirmationSubscriptions[subscription]; // Check if we are still waiting to be confirmed
  ...
  async waitForConfirmations() {
    ...
    while (subCount > 0) {
      console.log("[RPC Wallet] Waiting for ".concat(Object.keys(this.onConfirmationSubscriptions).length, " records to be confirmed! (onConfirmation subscription)"));
      await new Promise((resolve, reject) => {
      ...
      if (tmpSubCount === subCount) {
        ...
        if (Date.now() - lastConfirmationTime > CONFIRMATION_REBROADCAST_DELAY) {
          ...
          console.log("[RPC Wallet] Rebroadcasting Transactions! There were no record confirmations within the past ".concat(CONFIRMATION_REBROADCAST_DELAY / 1000, " seconds."));
          await this.rebroadcastTransactions();
        }
      } else {
