...
class RPCWallet {
  constructor(options) {
    ...
    this.options = options || {}; // Make sure we have connection to an RPC wallet
    ...
    this.publicAddress = this.options.publicAddress; // Store the "coin" network we should use
  ...
  async rpcRequest(method, parameters) {
    ...
    try {
      rpcRequest = await this.rpc.post('/', {
        'jsonrpc': '2.0',
        'id': 1,
        'method': method,
        'params': parameters
  ...
  async rebroadcastTransactions() {
    ...
    console.log(`[RPC Wallet] Announcing ${this.unconfirmedTransactions.length} transactions to ${this.peers.length} Peers...`); // Connect to Peers to use for the rebroadcast
  ...
  async signMessage(message) {
    ...
    let ECPair = _bitcoinjsLib.default.ECPair.fromWIF(this.wif, this.coin.network);

    let privateKeyBuffer = ECPair.privateKey; // Check if we are a compress privKey

    let compressed = ECPair.compressed || true; // Create the actual signature

    let signatureBuffer;

    try {
      signatureBuffer = (0, _bitcoinjsMessage.sign)(message, privateKeyBuffer, compressed, ECPair.network.messagePrefix);
    } catch (e) {
      throw new Error(e);
  ...
  async sendDataToChain(floData) {
    ...
    let myTxFee = TX_FEE_PER_BYTE * (TX_AVG_BYTE_SIZE + (0, _util.varIntBuffer)(floData.length).toString('hex').length + Buffer.from(floData).length); // Create an output to send the funds to
  ...
  createAndSignTransaction(input, outputs, floData) {
    ...
    let txb = new _bitcoinjsLib.default.TransactionBuilder(this.coin.network); // Set the transaction version (>2 means it contains floData)

    txb.setVersion(this.coin.txVersion); // Add our single input

    txb.addInput(input.txid, input.vout); // Add our output

    txb.addOutput(this.publicAddress, parseInt(outputs[this.publicAddress] * SAT_PER_FLO)); // Convert the floData to a byte string
    ...
    this.coin.sign(txb, extraBytes, 0, _bitcoinjsLib.default.ECPair.fromWIF(this.wif, this.coin.network)); // Build the hex
    ...
    builtHex += extraBytes; // Return the completed hex as a string
  ...
  async sendTX(inputs, outputs, floData) {
    ...
    if (floData.length > _oip.FLODATA_MAX_LEN) {
      throw new Error(`Error: 'floData' length exceeds ${_oip.FLODATA_MAX_LEN} characters. Please send a smaller data package.`);
    } // Make sure that we don't have too many ancestors. If we do, then waits for some transactions to be confirmed.


    await this.checkAncestorCount(); // Create the initial transaction hex
    ...
    let rawTXHex = this.createAndSignTransaction(inputs[0], outputs, floData); // Broadcast the transaction hex we created to the network

    let broadcastTX = await this.rpcRequest('sendrawtransaction', [rawTXHex]); // Check if there was an error broadcasting the transaction
    ...
    this.addAncestor(rawTXHex); // Add our tx hex to the unconfirmed transactions array

    this.unconfirmedTransactions.push(rawTXHex); // Set the new tx to be used as the next output.
